...
const cloneJDButton = () => {
  ...
  if (pathname.includes("the-financial-controller-you-didnt-know-you-needed")) {
    ...
    const targetDivGT = getElementRef(Selectors.dynamicEmbedGT);
    const sourceDivJD = getElementRef(Selectors.copyComponentJD);
    const sourceDivSN = getElementRef(Selectors.copyComponentSN);
    const sourceDivGT = getElementRef(Selectors.copyComponentGT);

    targetDivJD.appendChild(sourceDivJD);
    targetDivSN.appendChild(sourceDivSN);
    targetDivGT.appendChild(sourceDivGT);
  }
};
...
const hidePopup = () => {
  const mainPopupEle = getElementRef(Selectors.mainJDPopup);
  if (mainPopupEle) {
